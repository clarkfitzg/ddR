...
setMethod("get_parts", signature("ddR_RDD", "missing"), function(x, ...) {
    ...
    lapply(index, function(b) {
        new("ddR_RDD", RDD = x@RDD, partitions = x@partitions[b], dim = as.integer(x@psize[b, 
            ]), psize = matrix(x@psize[b, ], nrow = 1, ncol = length(x@psize[b, ])))
    })
})

#' @export
setMethod("get_parts", signature("ddR_RDD", "integer"), function(x, index, ...) {
    lapply(index, function(b) {
        new("ddR_RDD", RDD = x@RDD, partitions = x@partitions[b], dim = as.integer(x@psize[b, 
            ]), psize = matrix(x@psize[b, ], nrow = 1, ncol = length(x@psize[b, ])))
    })
...
#' @export
setMethod("do_collect", signature("ddR_RDD", "integer"), function(x, parts) {
    if (identical(x@partitions, parts) && length(parts) > 1) {
        temp <- SparkR:::collect(x@RDD, flatten = FALSE)
        if (is.dlist(x)) {
            do.call(c, lapply(temp, function(x) lapply(x, function(y) y[[2]])))
        } else {
            # Use nparts to stitch together here
            ind <- 0
            temp <- SparkR:::collect(mapPartition(x@RDD, function(p) {
                lapply(p, function(o) o[[2]])
            }), flatten = FALSE)
            ...
            for (i in seq(1, nparts(x)[[1]])) {
                ...
                for (j in seq(1, nparts(x)[[2]])) {
                  ...
                  if (is.dframe(x)) {
                    ...
                    chunk <- matrix(unlist(temp[[ind]]), psize(x, ind)[[1]], psize(x, 
                      ind)[[2]])
        ...
        if (is.dlist(x)) 
            ...
            if (is.dframe(x)) {
                # TODO: Optimize and set real names
                df.names <- lapply(seq(length(temp)), function(x) paste0("V", as.character(x)))
                df <- do.call(data.frame, temp)
                names(df) <- df.names
                df
            } else {
                matrix(unlist(temp), psize(x, parts)[[1]], psize(x, parts)[[2]])
            }
...
setMethod("combine", signature(driver = "ddR_RDD", items = "list"), function(driver, 
    items) {
    ...
    if (is.matrix(dims)) 
        dims <- colSums(dims) else dims <- sum(dims)
